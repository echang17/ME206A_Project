...
def callback(message,args):
  ...
  try:
    ...
    # The object should be flat, so it should span across the x-y plane in the base frame
    # of the sawyer robot.

    # For each ar tag, we have redundancy computed in my proposed changes.
    # We still must have AT LEAST ONE ar tag visible (from ar tags 1-4, so not 
    # inclusive of the human hand ar tag).
